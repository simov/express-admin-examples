...
app.get(/\/stats(?:\/(\w+))?/, (req, res, next) => {
  ...
  queryDatabase(db, month, (err, rows, total) => {
...
function queryDatabase (db, month, done) {
  // find the first and last date of the selected month (all test dates are in 2012)
  var date  = moment('2012-'+month, 'YYYY-MMMM')
  var first = date.startOf('month').format('YYYY-MM-DD')
  var last  = date.endOf('month').format('YYYY-MM-DD')
  ...
  db.client.query(sql, (err, items) => {
    ...
    db.client.query(sql, (err, sum) => {
      if (err) return done(err)
      
      // queries results
      var rows  = items
      var total = sum[0].total

      done(null, rows, total)
    })
  })
}
